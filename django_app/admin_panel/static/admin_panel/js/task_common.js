...
const TaskCommon = {
    ...
    selectAllInGroup: function(groupName, currentGroup, baseOptions, selectedIds, addTargetRow, removeTargetRow, refreshTomSelect) {
        ...
        const groupItems = baseOptions.filter(opt => {
            if (currentGroup === 'country') {
                const optCountry = opt.country || 'Без страны';
                return optCountry === groupName;
            } else if (currentGroup === 'category') {
                const optCategory = opt.category || 'Без группы';
                return optCategory === groupName;
            }
            return false;
        });
        ...
        if (allSelected) {
            ...
            groupItems.forEach(item => {
                if (selectedIds.has(String(item.value))) {
                    removeTargetRow(item.value, false); // false - без подтверждения
                }
            });
        } else {
        ...
        // Обновляем только чекбоксы, не перестраивая весь список
        setTimeout(refreshTomSelect, 0);
    },
    ...
    handleDropdownClick: function(ev, ts, selectedIds, addTargetRow, removeTargetRow, refreshTomSelect, currentGroup, baseOptions) {
        ...
        if (groupHeader) {
            ...
            this.selectAllInGroup(groupName, currentGroup, baseOptions, selectedIds, addTargetRow, removeTargetRow, refreshTomSelect, ts);
            return;
        }
        ...
        this.toggleSelection(id, selectedIds, addTargetRow, removeTargetRow, refreshTomSelect);
    },

    // Переключение выбора элемента
    toggleSelection: function(id, selectedIds, addTargetRow, removeTargetRow, refreshTomSelect) {
        if (selectedIds.has(id)) {
            removeTargetRow(id, false); // false - без подтверждения при клике на чекбокс
        } else {
            addTargetRow(id);
        }
        // Обновляем только чекбоксы, не перестраивая весь список
